...
def notify_of_event(messages):
    url = os.getenv("ENV_NOTIFY_URL_ENDPOINT")
    dest = os.getenv("ENV_NOTIFIER_DESTINATION")
    if not url or not dest:
        return

    requests.post(url, json={"to": dest, "messages": messages})
...
def update_user_credits(user_id, credits_to_buy):
    ...
    notify_of_event([f"User {user['email']} bought {credits_to_buy}"])
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse("Error in webhook " + str(e), status_code=400)

...
def analyze_read(blob, blob_name, PRICE_PER_1000_PAGES, user_credits):
    ...
    if user_credits < credits_to_pay:
        raise NotEnoughCreditsError(
            "Not enough credits to process this pdf: price is "
            + str(credits_to_pay)
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.error(
                f"Error in form recognizer endpoint. Attempt {attempt+1} of {max_retries} failed with error: {e}"
            )
        ...
        all_paragraphs.append(
            {
                ...
                "page_number": paragraph.bounding_regions[
                    0
                ].page_number,  # + page_number_base,
                "file_name": blob_name,
                "bounding_box": [
                    [
                        {"x": point.x, "y": point.y}
...
def combine_and_clean_paragraphs(paragraphs):
    ...
    while i < len(cleaned_paragraphs):
        ...
        if i < len(cleaned_paragraphs) - 1:
            ...
            ):  # only combine paragraphs from the same file
                i += 1
                continue
            ...
            if page_number != next_page_number and not content.endswith("."):
                ...
                if bounding_box and next_bounding_box:
                    cleaned_paragraphs[i]["bounding_box"] = (
    ...
    logging.info(
        "Number of paragraphs after combining paragraphs split across pages: "
        + str(len(cleaned_paragraphs))
    )
    ...
    for i in range(len(paragraphs)):
        ...
        if len(paragraph["content"]) > 250 * 6:  # 250 words ish
            ...
            segments = split_paragraph(
                paragraph["content"],
                segment_length=min(len(paragraph["content"]) / 2, 250 * 5),
                overlap_length=100,
            )
...
def upsert_to_milvus(embeddings, metadata):
    collection = "aifilesearch"
    uri = os.getenv("ENV_MILVUS_URI")
    token = os.getenv("ENV_MILVUS_TOKEN")
    # use requests to upsert
    url = f"{uri}/v1/vector/insert"
    headers = {
    ...
    data = [
        {
            #"id": t[0], # auto id instead on milvus
            "vector": embeddings[i],
            "metadata": metadata[i],
        }
        for i in range(len(embeddings))
    ]
    logging.info("Upserting to Milvus")
    body = {
        "collectionName": collection,
        "data": data,
    }
    logging.info(f"Number of embeddings to upsert: {len(data)}")

    res = requests.post(url, headers=headers, json=body)
    if res.status_code != 200:
        logging.error("Error in upserting to Milvus: " + res.text)
        raise Exception("Error in upserting to Milvus: " + res.text)
    else:
        logging.info("Upserted to Milvus")
    ...
    for i in tqdm(range(0, len(paragraphs), batch_size)):
        ...
        for paragraph in paragraphs_batch:
            temp = {
                ...
                "file_name": paragraph["file_name"],
                "content": paragraph["content"],
                "bounding_box": paragraph.get("bounding_box", None),
                "namespace": namespace,
            }

            if "---split---" in paragraph["file_name"]:
                split_file_name, page_numbers = paragraph["file_name"].split(
                    "---split---"
                )
                start_page, end_page = page_numbers.replace(".pdf", "").split("-")
                temp["file_name"] = split_file_name + ".pdf"
                temp["page_number"] += int(
                    start_page
                )  # Adding the starting page number to the current page number
...
def extract_text_from_doc(
    blob, price_per_1000_pages, user_credits
    ...
    if user_credits < credits_to_pay:
        raise NotEnoughCreditsError(
            "User does not have enough credits to process this file."
        )
    ...
    while True:
        ...
        for project in projects:
            if project["namespace"] == namespace:
                ...
                if "files" in project:
                    if file_info["blob_name"] not in [
                        file["blob_name"] for file in project["files"]
                    ]:
        ...
        try:
            cosmos_container.upsert_item(
                user, etag=etag, match_condition=MatchConditions.IfNotModified
            )
            break
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    paragraphs, cost, credits_to_pay, num_pages = analyze_read(
        blob, blob_name, PRICE_PER_1000_PAGES, user_credits
    )
    ...
    logging.info(
        f"Cleaned paragraphs. Number of paragraphs now: {len(cleaned_paragraphs)}"
    )
